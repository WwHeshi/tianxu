...
PROMPT_VERSION = "bazi-report-v22-graph-first"
...
2. 使用审慎、概率性的中文表述，把内容明确定位为传统文化视角下的参考，不把推断写成事实。
3. 不作疾病诊断、寿命判断、灾祸断言，不给出确定性的法律、投资或医疗建议。
4. 每一节应具体对应输入命盘，避免空泛套话；若数据不足，应直接说明局限。
5. 分析当前运势前必须调用 calculate_fortune_at，并以返回的大运、流年和流月为权威事实；
   不得自行推算或扩展完整时间线。
6. 神煞仅作辅助参考，不得依据单一神煞作吉凶断言。
7. 明确区分输入中的确定性事实与基于事实的传统解释，不得声称引擎已经计算输入未提供的结论。
8. 不得在最终回答中展示内部推理或工具调用过程。
