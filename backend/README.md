...
引擎，因此四柱、大运、流年、节气边界和子初换日口径与原排盘接口一致。基础四柱本身不依赖
性别，但当前工具同时生成大运，所以 `gender` 为必填字段。
...
报告接口接收与排盘相同的出生输入，后端先校验并换算真太阳时间，再运行单工具 ReAct
循环：模型可以按需发起 `calculate_bazi_chart` Action，也可以直接返回 Final；调用工具时，
后端严格核对参数并执行确定性排盘，随后把精简命盘作为 Observation 回传。Observation 不包含完整大运
时间线，只保留当前大运、流年和流月。当前没有知识库、RAG、引文或对话历史。

管理员生成报告时同时返回 `debug_trace`，用于展示 ReAct 的 Action、工具执行、Observation、
Final 和输出校验链路，并按实际响应轮数保存每次模型请求快照。普通用户的成功和失败响应都不包含执行
