...
`birthplace` 只接收静态快照中的 `location_id`；客户端不提交名称、层级、经纬度或时区。
响应会返回后端核验后的 `region_code`、IANA `timezone` 和真实可变层级
`division_path`。地点时区仅作为元数据，本版不用于时间换算。
...
- 输入时间固定解释为北京时间，排盘使用根据出生地点代表点换算后的真太阳时。
