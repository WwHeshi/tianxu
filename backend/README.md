...
后端严格核对参数并执行确定性排盘，随后直接把工具原局结果作为 Observation 回传。当前大运、
流年和流月由报告接口从完整命盘中选取并直接写入用户提示词，不由工具计算或返回。当前
没有知识库、RAG、引文或对话历史。
...
工具 Observation 只包含原局，不包含大运、流年或流月。每轮最多调用一个工具，最多允许
10 次模型响应。后端会累加所有响应轮次和重试的 Token，
