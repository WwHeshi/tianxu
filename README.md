...
当前 MVP 提供用户与管理员账户、确定性排盘、当前大运/流年/流月展示、AI 报告和管理员评测中心。报告与评测共用单工具 ReAct 架构：模型可以按需调用确定性排盘工具，后端返回精简命盘 Observation，也可以直接生成 Final；执行链路按实际模型响应轮次展示。当前不包含知识库、RAG 或对话历史。
