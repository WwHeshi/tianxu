...
知识库采用动态全文阅读方式，不预先建立固定切片：每次 Agent 运行只把当前书目目录、搜索命中附近的少量上下文，以及 Agent 主动翻页读取的原文发送给模型，不会把整本 TXT 放入提示词。TXT 原文搜索仍使用精确匹配和游标翻页；规则图谱搜索则使用名称或别名精确匹配、中文 BM25 与本地向量召回的混合排序。
...
`search_rule_graph` 每次从当前 Neo4j 读取规则，并按四步检索：名称或别名精确匹配置顶，
中文 BM25 召回前 30 条，本地原版 `bge-base-zh-v1.5` 向量召回前 30 条，再用 RRF 合并
两个排名。最终每个查询仍只返回前 5 条规则，工具输入输出结构不变。模型通过 ONNX Runtime
在后端 CPU 本地批量运行，不依赖当前对话模型厂商；查询侧会附加 BGE 中文
命理规则检索指令，规则正文不附加指令。模型文件保存在
`external/models/bge-base-zh-v1.5`，Docker 镜像直接从本地构建上下文复制，不在构建时联网
下载。非量化 FP32 ONNX 由 Xenova 从 BAAI 原始权重转换，模型目录只保存在本机并由
`.gitignore` 排除，不使用 Git LFS。新环境需要人工从
`Xenova/bge-base-zh-v1.5` 的固定版本 `71e50dc531959f9e04ebf190ea25b00261a0a186`
取得 `onnx/model.onnx` 和 `tokenizer.json`，并按上述目录结构放置。Docker 构建不会校验
模型哈希。
规则文本向量缓存在后端进程内，规则新增或变化时只重算对应向量，服务重启时重新预热；
向量模型不可用时会自动退化为精确匹配和 BM25。
...
规则合并必须通过 `existing_rule_id` 明确指定：无效编号会被拒绝；未指定编号但名称或别名与
现有规则完全相同时，也会返回候选规则并要求 Agent 明确选择合并或改用可区分的新名称。
后端不会根据 BM25 或向量相似度自动合并规则。
