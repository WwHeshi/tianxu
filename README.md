...
当前 MVP 提供用户与管理员账户、确定性排盘、当前大运/流年/流月展示、AI 报告和管理员评测中心。报告与评测共用通用工具调用执行器，并分别通过显式白名单获得工具权限；评测 Agent 可按需调用确定性排盘工具，报告 Agent 还可调用 `calculate_fortune_at` 获取报告基准时点的大运、流年和流月。当前不包含知识库、RAG 或对话历史。
