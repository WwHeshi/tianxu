...
当前 MVP 提供用户与管理员账户、确定性排盘、当前大运/流年/流月展示、AI 报告和管理员评测中心。报告与评测共用通用工具调用执行器，并分别通过显式白名单获得工具权限；目前二者只允许按需调用确定性排盘工具，后端返回精简命盘 Observation，也可以直接生成 Final。后端已提供 `calculate_fortune_at`，供未来有明确指定日期查询需求的 Agent 注册使用；它只返回命中时点的大运、流年和流月，不返回完整时间线。当前不包含知识库、RAG 或对话历史。
